...
class FrontMatter<T> {
	...
	constructor(mdcontent: string) {
		...
		this._metaIndices = this._lines.reduce(
			this.findMetaIndices,
			[] as number[],
		);
	}
	/**
	...
	/**
	 * Retrieves and parses the data from the provided 'linesPros' based on the metadata indices.
	 *
	 * @template T - The type of data to be returned.
	 * @param linesPros - An object containing 'lines' as an array of strings and 'metaIndices' as an array of numbers.
